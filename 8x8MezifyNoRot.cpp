...
#define COL 8
  ...
  void wallSaver(pair<int,int>temp,bool a[4]){ //LHR
  cout<<headc<<" orientation bolte !\n";
    celler[temp.first][temp.second].wall[0]=a[0]; //top
    celler[temp.first][temp.second].wall[1]=a[1]; //bottom
    celler[temp.first][temp.second].wall[2]=a[2]; //right
    celler[temp.first][temp.second].wall[3]=a[3];
...
void solve(){
  ...
  while(true){
        ...
        int bringVal=bringTheval(cell[cur.first][cur.second]);
        if(bringVal>=cell[cur.first][cur.second]){
